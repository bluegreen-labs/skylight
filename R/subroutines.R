...
altaz <- function(DS, H, SD, CI, SI, DR, RD){
  ...
  # vectorize using ifelse statement
  AZ <- ifelse(Q < 0.0, AZ + 180.0, AZ)

  # vectorize using ifelse statement
  AZ <- ifelse(AZ < 0.0, AZ + 360.0, AZ)
...
refr <- function(H,DR){
  ...
  HA <- ifelse(H < (-5.0/6.0),
              H,
              H+1.0/(tan((H+8.6/(H+4.42))*DR))/60.0
  )
  return(HA)
}
